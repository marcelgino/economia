...
import React from 'react';
import { Link, useNavigate } from 'react-router-dom';
import { TrendingUp, PenTool, LogOut, Search } from 'lucide-react';
import { supabase } from '../lib/supabase';
    ...
    <nav className="fixed top-0 left-0 right-0 z-50 glass border-b border-white/5 px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        <div className="flex items-center gap-4 flex-1 justify-end">
            ...
            {isSearchOpen ? (
              <form onSubmit={handleSearch} className="w-full">
                <input 
                  ...
                  onBlur={() => !searchQuery && setIsSearchOpen(false)}
                  placeholder="Rechercher un article..."
                  className="w-full bg-white/5 border border-white/10 rounded-full py-2 pl-10 pr-4 text-sm focus:outline-none focus:border-[#F27D26]"
                />
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
              </form>
            ) : (
              <button onClick={() => setIsSearchOpen(true)} className="p-2 hover:bg-white/5 rounded-full transition-colors">
                <Search className="w-5 h-5 text-white/60" />
              </button>
            )}
          </div>
          ...
          {user ? (
            <div className="flex items-center gap-4">
              <Link to="/publish" className="bg-white text-black px-4 py-2 rounded-full text-sm font-semibold hover:bg-[#F27D26] hover:text-white transition-all">Publier</Link>
              <button onClick={() => supabase.auth.signOut()} className="text-white/60 hover:text-white"><LogOut className="w-5 h-5" /></button>
            </div>
          ) : (
            <Link to="/auth" className="bg-white text-black px-6 py-2 rounded-full text-sm font-semibold hover:bg-[#F27D26] hover:text-white transition-all">Connexion</Link>
          )}
        </div>
      </div>
    </nav>
