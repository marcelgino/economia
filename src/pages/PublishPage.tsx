  ...
  const handlePublish = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const articleData = {
        ...
        content,
        category,
        image_url: imageUrl || `https://picsum.photos/seed/${Date.now()}/1200/800`,
        author_id: user.id,
        author_name: user.user_metadata.full_name || user.email?.split('@')[0]
      };
      ...
      <main className="max-w-4xl mx-auto px-6">
        ...
        <form onSubmit={handlePublish} className="space-y-8">
          <div className="space-y-6 bg-[#111111] border border-white/5 rounded-3xl p-8">
            <div>
              <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/40 mb-3">
                <Type className="w-3 h-3" /> Titre de l'article <span className="text-[#F27D26]">*</span>
              </label>
              <input 
                type="text"
                value={title}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/40 mb-3">
                  Catégorie <span className="text-[#F27D26]">*</span>
                </label>
                <select 
                  value={category}
                  onChange={(e) => setCategory(e.target.value)}
                  className="w-full bg-white/5 border border-white/10 rounded-xl py-3 px-4 text-sm focus:outline-none focus:border-[#F27D26] appearance-none"
                  required
                >
                  <option value="Macroéconomie">Macroéconomie</option>
                  <option value="Finance">Finance</option>
            ...
            <div>
              <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/40 mb-3">
                <FileText className="w-3 h-3" /> Résumé court <span className="text-[#F27D26]">*</span>
              </label>
              <textarea 
                value={summary}
                onChange={(e) => setSummary(e.target.value)}
            ...
            <div>
              <label className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/40 mb-3">
                Contenu (Markdown supporté) <span className="text-[#F27D26]">*</span>
              </label>
              <textarea 
                value={content}
                onChange={(e) => setContent(e.target.value)}
